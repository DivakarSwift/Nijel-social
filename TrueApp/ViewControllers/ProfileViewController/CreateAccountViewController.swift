...
//        bottomLayer.backgroundColor = UIColor(red: 50/255, green: 50/255, blue: 25/255, alpha: 1).cgColor
    ...
    func showEmailAlert(success:@escaping (Bool)->()) {
        if self.selectedImage == nil || self.fullNameTextField.text?.trimmingCharacters(in: .whitespaces) == nil || self.fullNameTextField.text?.trimmingCharacters(in: .whitespaces) == "" {
            let alertController = UIAlertController(title: "Warning", message: "Set image and name to created profile", preferredStyle: UIAlertController.Style.alert)
        ...
        alert.addAction(UIAlertAction(title: "Send", style: .default, handler: { [weak alert] (_) in
            ...
            if !self.isValidPhone(testStr: textField.text!) {
                let alert = UIAlertController(title: "Warning", message: "Phone number format is not valid", preferredStyle: UIAlertController.Style.alert)
                alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { action in
                    self.showEmailAlert(success: { (completion) in
                        success(false)
                    })
